...
* TACC-hosted PostgreSQL database that retains production ready, validated data within ta- bles. For complex queries, materialized views are used to maintain query performance;
* Web Data Connect that allows Tableau to retrieve data from the production Postgres Tables and materialized views;
* Production, Pre-production and Development Postgres instances, with access for Democ- ratizing Data to develop and push data directly to the Postgres environment with a rollover- based deployment process for new production data and schema changes;
